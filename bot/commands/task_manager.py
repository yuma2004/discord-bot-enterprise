...
class TaskManagerCog(commands.Cog):
    ...
    async def add_task(self, ctx, *, task_info: str):
        """新しいタスクを追加する
    ...
    async def list_tasks(self, ctx, status: str = None):
        """タスク一覧を表示する
        ...
        # タスク一覧を取得
        tasks = task_repo.get_user_tasks(user['id'], status)
    ...
    async def progress_task(self, ctx, task_id: int):
        """タスクを進行中にする
    ...
    def _parse_task_info(self, task_info: str) -> dict:
        """タスク情報を解析"""
        task_data = {
            'title': '',
            'description': '',
